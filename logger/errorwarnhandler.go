...
// WithAttrs returns a new ErrorWarnHandler with the given attributes.
func (h ErrorWarnHandler) WithAttrs(attrs []slog.Attr) slog.Handler {
	return &ErrorWarnHandler{Handler: h.Handler.WithAttrs(attrs), f: h.f}
}

// WithGroup returns a new ErrorWarnHandler with the given group name.
